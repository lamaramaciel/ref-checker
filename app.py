...
if run_all:
    start_row = 1
    end_row = len(df)
    st.info(f"All **{len(df)} rows** selected · {len(ref_pairs)} columns = **{len(df) * len(ref_pairs)} checks**")
    if len(df) * len(ref_pairs) > 250:
        st.warning(f"⚠️ {len(df) * len(ref_pairs)} checks exceeds the free tier daily limit of 250. Consider running in batches.")
else:
        ...
        for data_col, ref_col in ref_pairs:
            ...
            if not declared_clean or declared_clean in ("nan", "N/A", "/", ""):
                results.append({
                    ...
                    "Declared Value": declared_clean, "Reference": references_raw[:100],
            ...
            results.append({
                "Organization": org_name, "Column": data_col,
                "Declared Value": declared_clean, "Reference": references_raw[:120],
